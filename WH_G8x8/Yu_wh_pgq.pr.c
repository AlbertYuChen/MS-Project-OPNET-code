...
const char Yu_wh_pgq_pr_c [] = "MIL_3_Tfile_Hdr_ 171A 30A modeler 7 55392F49 55392F49 1 ECE-PHO309-01 chenyua 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 2b1a 1                                                                                                                                                                                                                                                                                                                                                                                                    ";
#include <string.h>
...
Packet * get_pk_in_queue(void) {
	...
	if (op_subq_empty(0) == OPC_FALSE){
		...
		pkptr = NULL;
		printf("PGQ %d get_pk_in_queue get NULL pointer\n", This_Node_Number);
	}
...
/*
	...
	do {
		...
		gen_Dest_Node_Number = gen_Dest_Node_Number + (helper * 4);
	}while(gen_Dest_Node_Number == This_Node_Number);
	...
	if( (int)Inter_Arrival_Time * 17 % 2 == 1){
	
		if(This_Node_Number == 9 ){
			gen_Dest_Node_Number = 1;
		}
		else if(This_Node_Number == 4){
	 		gen_Dest_Node_Number = 6;
		}
		...
		if(This_Node_Number == 4){
			gen_Dest_Node_Number = 1;
		}
		else if(This_Node_Number == 9){
	 		gen_Dest_Node_Number = 1;
		}
	...
	if( (int)Inter_Arrival_Time * 17 % 3 == 0){
	
		if(This_Node_Number == 5 ){
			gen_Dest_Node_Number = 6;
		}
		else if(This_Node_Number == 7){
	 		gen_Dest_Node_Number = 6;
		}
	
	}
	
	else if( (int)Inter_Arrival_Time * 17 % 3 == 1){
		
		if(This_Node_Number == 5 ){
			gen_Dest_Node_Number = 10;
		}
		else if(This_Node_Number == 7){
	 		gen_Dest_Node_Number = 10;
		}
		...
		if(This_Node_Number == 5 ){
			gen_Dest_Node_Number = 6;
	 		...
	 		gen_Dest_Node_Number = 2;
		}
	}
	
	
	if(This_Node_Number == 0){
		gen_Dest_Node_Number = 5;
	}
	else if(This_Node_Number == 4){
	 	gen_Dest_Node_Number = 1;
	}
	else if(This_Node_Number == 5){
	 	gen_Dest_Node_Number = 0;
	}
	else if(This_Node_Number == 1){
	 	gen_Dest_Node_Number = 4;
	}
 */
	...
	*/
	if(Being_Trans_or_Not == Not_Being_Trans) {
		...
		if(tmp_type != Flit_Type_Dest_Addr){
			printf("PGQ ERROR:Node:%d", This_Node_Number);
	...
	if (op_prg_odb_ltrace_active ("PGQ") == OPC_TRUE){
	printf("------------PGQ: Node:%d a new worm is comming!  Totalin the Queue:%d \n", This_Node_Number, Worm_Counter);
	}
...
void send_next_flit(){
	...
	if(Worm_Counter == 0 && Being_Trans_or_Not == Being_Trans){
		Being_Trans_or_Not = Not_Being_Trans;
	}
		
	else if (Worm_Counter > 0){
	
		if(Num_Flits_Left_to_Send == Worm_Gen_Len){
			pkptr = get_pk_in_queue();
			op_pk_nfd_get(pkptr, "type", &tmp_type);
			op_pk_nfd_get(pkptr, "data", &tmp_data);
		...
		// if the next flit is source head flit or worm length flit, then extract from the queue
		else if(Num_Flits_Left_to_Send == Worm_Gen_Len - 1){
			// pkptr = get_pk_in_queue();
			...
			Num_Flits_Left_to_Send--;
				...
				// if the next flit is source head flit or worm length flit, then extract from the queue
		else if(Num_Flits_Left_to_Send == Worm_Gen_Len - 2){
			// pkptr = get_pk_in_queue();
		...
		//Once receive remote intrpt of schedul_TF, send TF ASAP
			...
			Num_Flits_Left_to_Send = Worm_Gen_Len;
		...
		else if (Num_Flits_Left_to_Send > 0){
...
void load_ARnode_PGQ(void){
	...
	sprintf(RName, "C:\\Users\\chenyua\\OPNET_Project\\WH_G8x8\\sim_data\\ARnode_%d.txt", This_Node_Number);
...
void load_Rnode_PGQ(void){
	...
	sprintf(RName, "C:\\Users\\chenyua\\OPNET_Project\\WH_G4x4\\sim_data\\Rnode_%d.txt", This_Node_Number);
	if (!(Rinfile = fopen(RName, "r"))) {
...
void initialize_PGQ(void){
	...
	// Inter_Arrival_Distr = op_dist_load ("uniform", 1, 2);
	
	Inter_Arrival_Time = op_dist_outcome(Inter_Arrival_Distr);
	
	if (op_prg_odb_ltrace_active ("PGQ") == OPC_TRUE){
	printf("Worm_Gen_Rate: %f   Inter_Arrival_Time:%f\n", Worm_Gen_Rate, Inter_Arrival_Time);
	}
	...
	// if (This_Node_Number == 4 || This_Node_Number == 1 || This_Node_Number == 6 || This_Node_Number == 9 || This_Node_Number == 5)
	...
	*/
	...
	//if (This_Node_Number == 0 || This_Node_Number == 4 || This_Node_Number == 8 || This_Node_Number == 1)
	//	|| This_Node_Number == 1 || This_Node_Number == 2 || This_Node_Number == 3)
		
	// if (This_Node_Number == 1 || This_Node_Number == 4)
		
	//op_intrpt_schedule_self (op_sim_time () +  0.1, GEN_MSG_INTRPT_CODE);
		
	op_intrpt_schedule_self (op_sim_time () +  Inter_Arrival_Time + 1.0, GEN_MSG_INTRPT_CODE);
	...
	FIN_MT (Yu_wh_pgq ());
		...
		FSM_BLOCK_SWITCH
			...
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "Yu_wh_pgq [init enter execs]")
				...
				queue_size = op_stat_reg ("Queue Size", OPC_STAT_INDEX_NONE, OPC_STAT_GLOBAL);
